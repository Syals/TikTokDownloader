"""采集分页的 TikTok For You 推荐流内容并下载其媒体。
...
在仓库根目录运行：
    uv run python poc/explore/tiktok_foryou_poc.py
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / ".output" / "foryou" / "samples"
DEFAULT_DOWNLOAD_DIR = PROJECT_ROOT / ".output" / "foryou" / "downloads"
...
async def probe_profile(
    ...
    response = await client.get(
        ...
        headers=headers(cookie, "https://www.tiktok.com/"),
    )
...
async def fetch_recommend_page(
    ...
    response = await client.get(
        ...
        headers=headers(cookie, "https://www.tiktok.com/foryou"),
    )
    refresh_cookie(cookie, response)
    content_type = response.headers.get("content-type", "?")
    print(
